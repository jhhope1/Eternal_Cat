...
with open(os.path.join(data_path, 'object_to_chunkidx.json'), 'r', encoding='utf-8') as f:
    song_to_chunkidx = json.load(f)
with open(os.path.join(data_path, 'tag_to_chunkidx.json'), 'r', encoding='utf-8') as f:
    tag_to_chunkidx = json.load(f)
with open(os.path.join(data_path, 'title_to_chunkidx.json'), 'r', encoding='utf-8') as f:
    title_to_chunkidx = json.load(f)
...
with open_utf8(val_file, 'r') as f3, open_utf8(res_file, 'w') as f4:
    ...
    for st in range(0, len(val_list), batch_size):
        ...
        for j in range(st, ed):
            ...
            #chunk extraction and tensorization
            for song in song_set:
                chunk_list = [output_dim + x for x in song_to_chunkidx[song]]
                input_one_hot[j - st][chunk_list] += 1
            for tag in tag_set:
                chunk_list = [output_dim + x for x in tag_to_chunkidx[tag]]
                input_one_hot[j - st][chunk_list] += 1
            chunk_list = [output_dim + x for x in title_to_chunkidx[val_list[j]['plylst_title']]]
            input_one_hot[j - st][chunk_list] += 1
