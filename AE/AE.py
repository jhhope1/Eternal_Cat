...
def batch_of(batch_size):
    ...
    denoised_one_hot = torch.zeros(batch_size, input_dim)
    for idx, idx_list in enumerate(playlist_idx[ch]):
        if len(idx_list) > 0:
            denoised_one_hot[idx] = torch.zeros(input_dim).scatter(0, torch.tensor(idx_list), 1)

    noised_one_hot = (torch.rand(batch_size, input_dim) > 1 - noise_p)*denoised_one_hot
    
    sample = {'input_one_hot':noised_one_hot, 'target_one_hot':denoised_one_hot}
    return sample
...
def test_accuracy():
  ...
  with torch.no_grad():
    ...
    for data in test_loader:
        ...
        total_lostsong += torch.sum(diff.reshape(-1))
