...
class AutoEncoder(nn.Module):
  def __init__(self, layer_sizes, nl_type='selu', is_constrained=True, dp_drop_prob=0.0, last_layer_activations='sigmoid', symmetric = True):
    """
    ...
    :param last_layer_activations: (default: sigmoid) Whether to apply activations on last decoder layer
    """
    ...
    self.encode_bn = nn.ModuleList([torch.nn.BatchNorm1d(layer_sizes[0][i+1]) for i in range(len(layer_sizes[0])-2)])
    self.decode_bn = nn.ModuleList([torch.nn.BatchNorm1d(layer_sizes[1][i+1]) for i in range(len(layer_sizes[1])-2)])
