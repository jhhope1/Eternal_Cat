...
song_vec = {song : (np.asarray(vec[0]), vec[1]) for song, vec in song_vec1.items()} ## vec, frequency
...
def find_and_append(data, song_extend_len, tag_extend_len):
    ...
    print(i, "was used outof ", len(data))
    song_possible_list = []
    tag_possible_list = []
    
    for every_song, vec in song_vec.items():
        song_possible_list.append((np.linalg.norm(vec[0] - playlist_vec)/(1 + math.log(song_vec[every_song][1] + 3)), every_song))
    for every_tag, vec in tags_vec.items(): 
        tag_possible_list.append((np.linalg.norm(vec[0] - playlist_vec)/(1 + math.log(tags_vec[every_tag][1] + 3)), every_tag))
    ...
    for real, song in song_possible_list:
        ...
        if len(tdata['songs']) > song_extend_len:
            break
...
for i in range(len(test_set)):
    ...
    answer = find_and_append(input_data, 100, 100)
    right_song_ctr = 0
    right_tag_ctr = 0
