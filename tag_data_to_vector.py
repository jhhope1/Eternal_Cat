...
m_used = {tag : max([val for tag2, val in tag_weighted_graph[tag].items()]) for tag, real in tag_pq_exi.items()}
for i in range(SZ):
    tag1 = tag_pq[i]
    ...
    for j in range(SZ):
        ...
        if tag_weighted_graph[tag1].get(tag2) == None:
            ...
            x = tag_weighted_graph[tag1][tag2]/min(m_used[tag1], m_used[tag2])
...
embedding = manifold.SpectralEmbedding(n_components = lapEig, affinity = 'precomputed')

X_transformed = np.array(embedding.fit_transform(np.asarray(aff_mat)))
...
for i in range(Y.shape[0]):
    if i < 80:
        ...
        continue
    if i >= SZ:
        plt.text(Y[i,0], Y[i,1], song_name[i-SZ][0:20], FontProperties = fontprop, color = (1,0,0))
